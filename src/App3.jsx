...
/**
 * SLICE 6 — Readout + export pack (deploy together)
 *
 * Adds these routes (together):
 * - /cases/:caseId/readout
 * - /reports
 ...
 *
 * Goal:
 * - handoff artifact that survives politics
 * Why together:
 * - readout must point to a Report object
 * - report must support export + frozen receipt bundle
 * - otherwise you get screenshot games
 */

/* =========================================================
   A) ICONS — add to lucide-react imports
   ========================================================= */
import { FileOutput, FolderArchive, FileSearch, FileDown, ClipboardSignature } from "lucide-react";

/* =========================================================
   B) DATA — extend seed + case creation to include reports
   =========================================================
   Add:
   - reports array in makeSeedData()
   - case.readoutReportId optional pointer (nice UX)
   ========================================================= */

// In makeSeedData(), add a reports array (and optionally link it from the demo case)
function makeSeedData() {
  // ... your existing seed objects ...
  ...
  // Optional: link report from case so /cases/:id/readout can “open latest”
  const cases = [
    {
  ...
  return { ...existingReturn, reports };
}

// In CaseNewPage createCase(), add:
const c = {
  // ... existing fields ...
  readoutReportId: null,
};

// Helper ID function already exists (makeId). Use makeId("rep") for new reports.

/* =========================================================
   C) ROUTER — add Slice 6 routes to parseRoute()
   =========================================================
   Add:
   - /cases/:caseId/readout
   - /reports
...
function parseRoute(route) {
  ...
  if (parts[0] === "cases") {
    ...
    // ... your existing /cases/:caseId/* routes ...
...
/* =========================================================
   D) MAIN — add memos + page renders + fallback whitelist entries
   ========================================================= */

// In main component memos:
...
/* =========================================================
   E) CASE DETAIL — add “Readout” button (recommended)
   Put inside CaseDetailPage button row
   ========================================================= */
...
/* =========================================================
   F) NAV — optional Reports link in sidebar
   (if you want it visible outside case flow)
   ========================================================= */
// Add to NAV:
{ key: "/reports", label: "Reports", icon: FolderArchive },

/* =========================================================
   G) PAGES — paste below your Slice 5 pages
   ========================================================= */
...
/* =========================================================
   H) CSS — optional: reuse existing styles
   (You already have .textarea; ensure it exists in your Style block)
   ========================================================= */
/*
textarea.textarea style already present in your earlier versions.
If not, add:
.textarea { ... }  // same as your prior POC
*/
